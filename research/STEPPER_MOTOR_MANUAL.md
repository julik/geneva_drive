...
The execution of the steps takes place inside the `PerformStepJob#perform` method, so the queueing options of that job may be important. You may also want to dispatch jobs for specific Journeys into specific queues. To facilitate that (for example, to use a specific queue or set a custom priority), Journey have the `set_step_job_options` class method. It sets the job options for a particular `Journey` subclass, merging into the superclass options. Call `set_step_job_options` with any options you would pass to ActiveJob’s `set` method. For example:

```ruby
class HighPriorityJourney < StepperMotor::Journey
  set_step_job_options queue: "critical", priority: 0

  step :important_step do
    # ...
  end
end
```

All jobs enqueued for this journey (including retries and scheduled steps) will use the specified queue and priority. The step job options are stored at the class level and accessed through the `step_job_options_for_next_step` method on journey instances. Schedulers automatically retrieve these options when enqueuing jobs and merge them with their own scheduling options (like `wait_until`). You can also override the `step_job_options_for_next_step` instance method for even more control. 

The options are class-inheritable and can be overridden in subclasses:

```ruby
class ParentJourney < StepperMotor::Journey
  set_step_job_options queue: "parent_queue", priority: 5
end

class ChildJourney < ParentJourney
  set_step_job_options queue: "child_queue", priority: 10
end
```

If you do not set any job options, jobs will be enqueued with the default queue and priority of your ActiveJob backend. Any option supported by ActiveJob's `set` method can be used - `queue` and `priority` to name a few. Timing options, however, are controlled by the scheduler - those are `wait` and `wait_until`.

For more details, see the [ActiveJob documentation on custom job options](https://api.rubyonrails.org/classes/ActiveJob/Core/ClassMethods.html#method-i-set).